...
import { useGetApprovedRequestsQuery, useGetPendingRequestsQuery } from "@/app/api/requestApiSlice";
...
export default function ViewApprovedRequestsPage() {
  const navigate = useNavigate();
  const isFinance = useIsFinance();
  const { user } = useAppSelector((state: RootState) => state.auth);
  const toastId = "unauthorized-error";
  const { data, isLoading, error } = useGetApprovedRequestsQuery();
  ...
  useEffect(() => {
    setBreadcrumbs(["Início", "Solicitação", "Aprovados"]); // Define os breadcrumbs da página atual
  }, [setBreadcrumbs]);
      ...
      <Card className="col-span-12 flex flex-col shadow-none border-0">
        <CardHeader>
          <CardTitle className="text-2xl w-full flex justify-between">
            Solicitação aprovadas
          </CardTitle>
          <CardDescription>Solicitações aprovadas para emissão. </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading
            ? (
